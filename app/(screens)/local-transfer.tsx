  ...
  const handleSwipeSuccess = () => {
    ...
    router.push({
      ...
      params: {
        ...
        bankIcon: banks.find(bank=> bank.name === details.bankName)?.icon || 'bank'
      }
    });
  ...
  React.useEffect(() => {
    if (details.bankName && details.accountNumber) {
      ...
      if (holder) {
        ...
        // Set a 5-second timer before showing the account holder
        const timer = setTimeout(() => {
          setAccountHolder(holder);
          setIsLoadingAccountHolder(false);
        }, 5000);

        // Clear the timer if the component unmounts or details change
