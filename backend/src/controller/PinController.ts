...
const COMMON_PINS = [
  ...
  '1234', '4321', '1004', '2000', '1212', '1122', '1313', '2001', '1010', '1221'
];
...
export const createPin = async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    if (COMMON_PINS.includes(pin)) {
      return res.status(400).json({ success: false, message: "This PIN is too common. Please choose a more secure PIN" });
    }

    if (/(\d)\1{3}/.test(pin) || '0123456789'.includes(pin) || '9876543210'.includes(pin)) {
      return res.status(400).json({ 
        success: false, 
        message: "This PIN pattern is not allowed. Please choose a more complex PIN." 
      });
    }
